...
<div>
    <SideBar/>
		...
		<td>{{project.pwd}}</td>
                                  ...
                                  <td>
                                  <router-link :to="`/UtilisateurEdit/${project.id}`" class="btn btn-success mx-1">Edit</router-link>
		<router-link :to="`/UtilisateurShow/${project.id}`" class="btn btn-success mx-1">Show</router-link>
...
axios.post(`Zaby/tocrudutilisateur.do?offset=${offset}`,formData)
          .then(response => {
...
formData.append('role',sessionStorage.getItem('role'));
                  ...
                  axios.post(`/Zaby/deleteutilisateur.do?id=${id}`,formData)
                  .then( response => {
                                if(Object.keys(response.data).length === 0 ){
              this.$router.push('/');
            }
