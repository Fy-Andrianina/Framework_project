...
public class Language {
    ...
    public String generateController(Entity entity, Database database, Credentials credentials, String projectName)
        ...
        /////////////////////////// si user ajout des login et signout
        ControllerMethod[] controllerMethods = getController().getControllerMethods();

        if (entity.getTableName().equals("utilisateur")) { // faut pas faire statique fyyyyyy
            ControllerMethod[] temp = new ControllerMethod[controllerMethods.length
                    + getController().getControllerUserMethods().length];
            int indice = 0;
            for (ControllerMethod m : controllerMethods) {
                temp[indice] = m;
                indice++;
            }

            for (ControllerMethod m : getController().getControllerUserMethods()) {
                temp[indice] = m;
                indice++;
            }
            controllerMethods = temp;
        }
        /////////////////////////////////////////////////////////////
        for (ControllerMethod m : controllerMethods) {
            methodAnnotes = "";
